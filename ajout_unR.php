...
if(isset($valider)){
    ...
    if(empty($salaire) or $salaire<0) {
        echo "<mark>nombre invalide ou champ laisser vide</mark>"."<br/>";
    }
      else{
    ...
    $pdo= new PDO("mysql:host=localhost;dbname=ressource","root","");
       ...
       $revenu=$pdo->prepare("insert into revenu(date,salaire) values(now(),?)");
       $revenu->execute(array($salaire));
       $revenu->execute();
    ...
    <table border=1>
        <tr>
          <td>salaire</td>
           ...
           <tr> 
             <td><input type="text"  classe="cl"name="salaire" ></td>
        </tr>
